...
public record UploadPartialPicturePacket(UUID uuid, byte[] bytes, int bytesLeft) {
    private static final Identifier ID = Identifier.fromNamespaceAndPath("camerapture", "upload_partial_picture");
    ...
    public static final StreamCodec<ByteBuf, UploadPartialPicturePacket> STREAM_CODEC = StreamCodec.of(
            (buf, packet) -> {
                ...
                ByteBufCodecs.byteArray(TransportLimits.CLIENT_SECTION_SIZE).encode(buf, packet.bytes());
                ByteBufCodecs.VAR_INT.encode(buf, packet.bytesLeft());
            },
            buf -> {
                UUID uuid = UUIDUtil.STREAM_CODEC.decode(buf);
                byte[] bytes = ByteBufCodecs.byteArray(TransportLimits.CLIENT_SECTION_SIZE).decode(buf);
