...
public record DownloadPartialPicturePacket(UUID uuid, PictureQuality quality, byte[] bytes, int bytesLeft) {
    private static final Identifier ID = Identifier.fromNamespaceAndPath("camerapture", "download_partial_picture");
    ...
    public static final StreamCodec<ByteBuf, DownloadPartialPicturePacket> STREAM_CODEC = StreamCodec.of(
            (buf, packet) -> {
                ...
                ByteBufCodecs.byteArray(TransportLimits.SERVER_SECTION_SIZE).encode(buf, packet.bytes());
            ...
            buf -> {
                ...
                byte[] bytes = ByteBufCodecs.byteArray(TransportLimits.SERVER_SECTION_SIZE).decode(buf);
